...
- **Roles**: only `SUPER_ADMIN` (platform-wide, `lembagaId: null`) and `LEMBAGA_ADMIN` (tenant staff) exist in the `Role`/RBAC table. There is no `DONATUR`, `RELAWAN`, or `FINANCE` role.
